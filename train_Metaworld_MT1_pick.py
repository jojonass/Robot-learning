...
EXPERIMENT_BATCH_ID = os.environ.get("BATCH_ID")
...
def make_env(task_name, env_id, base_seed):
    def _init():
        ...
        # 2. Wrap with SuccessFixer (directly touching raw env)
        env = SuccessFixer(env, mt1.train_tasks, seed)
        # 3. Wrap with Monitor (on the outside)
        env = Monitor(env)
        return env
    return _init
...
class SuccessFixer(gym.Wrapper):
    ...
    def reset(self, **kwargs):
        # FIX: Pick integer index to avoid UnicodeDecodeError with Task objects
        idx = self.np_random.integers(0, len(self.tasks))
        task = self.tasks[idx]
        
        # Now works because SuccessFixer is inside the Monitor
